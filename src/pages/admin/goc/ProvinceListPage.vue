<template>
    <div>
        <FormHeader :title="$t('city_list')">
            <el-button type="primary" @click="dialogFormVisible = true, isEditing = false, formHelper.clearForm(ruleFormRef)">{{ $t('create_new') }}</el-button>
        </FormHeader>
        <el-table :data="cityData" border v-loading="pageLoading" header-cell-class-name="my-table-header"
            ...
            <el-table-column prop="Image" :label="$t('image')" width="120">
                <template #default="{ row }">
                    <div class="w-[100px] max-h-[100px] rounded-md overflow-hidden ">
                        <img v-if="row.CityImage !== ''" class=" h-full w-full  object-cover"
                            :src="`https://localhost:3001/api/ImageUpload/get/cities/${row.CityImage}`" alt="">
                        <img v-else class=" h-full w-full  object-cover"
            ...
            <el-table-column fixed="right" align="center" :label="$t('actions')" width="100">
                <template #default="scope">
                    <el-button @click="openEditForm(scope.row.Id)" type="info" size="small" > {{ $t('edit') }} <el-icon class="el-icon--right"><Edit/></el-icon></el-button>
                </template>
            </el-table-column>
        </el-table>
            ...
            <el-form :model="cityRequestForm" :rules="rules" ref="ruleFormRef">
                ...
                <el-form-item :label="$t('city_image')" prop="CityImage">
                    <div class="flex w-full justify-between">
                        <div class=" max-w-[200px] max-h-[100px] rounded-md overflow-hidden border">
                            <img v-if="cityRequestForm.CityImage !== ''" class=" h-full w-full  object-contain"
                                :src="`https://localhost:3001/api/ImageUpload/get/cities/${cityRequestForm.CityImage}`"
                                alt="">
                            <img v-else class=" h-full w-full  object-contain" src="../../../assets//images/noImage.jpg"
                                alt="">
                        </div>
                        <div>
...
const { cityData, getAllCities, pageLoading, cityRequestForm,
    dialogFormVisible, rules, handleChange,
    onSubmitCreate, onSubmitUpdate,openEditForm,isEditing } = useCity();
